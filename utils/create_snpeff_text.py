...
def prepare_snpeff_run(ref_path_gb, locus, ref_path_fa, reference_name, output):
    ...
    with open("config/snpeff.config", "r", encoding="UTF8") as snpeff_config:
    ...
    if find == -1:
        subprocess.run(
            f"mkdir  config/data/{reference_name} -p", shell=True, check=False
        )
        subprocess.run(
            f"cat {ref_path_gb} > config/data/{reference_name}/genes.gbk",
            shell=True,
            check=False,
        )
        subprocess.run(
            f"cat {ref_path_fa} > config/data/{reference_name}/sequences.fa",
            shell=True,
            check=False,
        )
        ...
        subprocess.run(
            f"snpEff build -genbank {reference_name} -c config/snpeff.config > {output}",
            shell=True,
            check=False,
        )
    else:
