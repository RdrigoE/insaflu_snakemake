...
def write_fasta(dictionary, filename):
    ...
    with open(filename, "w", encoding="utf-8") as fasta:
...
def get_ref_adjusted_positions(alignment, positions, gene):
    """
...
if __name__ == "__main__":
    REFERENCE_GB = sys.argv[1]
    ALIGNMENT = sys.argv[2]
    OUTPUT = sys.argv[3]
    NAME = sys.argv[4]
    GENE_NAME = sys.argv[5]
    COVERAGE_FILE = sys.argv[6]

    write_fast_aa(REFERENCE_GB, ALIGNMENT, OUTPUT, NAME, GENE_NAME, COVERAGE_FILE)
