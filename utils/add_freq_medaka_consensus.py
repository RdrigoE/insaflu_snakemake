...
def add_structure_to_vcf(file_path, output_file):
    """
    ...
    with open(file_path, "r", encoding="UTF8") as invcf:
        for line in invcf:
            try:
                if line.startswith("#"):
                    continue
                ...
                if "snp" in info_dic["TYPE"]:
                    ...
                    if freq > 0.50:
                        ...
                        new_entry.append(
                            re.findall("(?<=medaka/)(.*?)(?=.vcf)", file_path)[0]
                        )  # ID
                ...
                continue  # ignore noexcept

    with open(output_file, mode="w", encoding="UTF8") as output_file_handler:
        f_writer = csv.writer(output_file_handler, delimiter=",")
...
if __name__ == "__main__":
    ...
    add_structure_to_vcf(FILE_PATH, OUTPUT_FILE)
