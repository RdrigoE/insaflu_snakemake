...
def get_abricate_info(path: str) -> list[str]:
    """
    ...
    with open(path, encoding="utf-8") as csv_handler:
        ...
        info_list: list[list[str]] = list(reader)

    genus_list: list[str] = []
    species_list: list[str] = []

    for entry_list in info_list:
        entry: str = entry_list[0]
        if "genus" in entry:
            genus: str = re.findall("(?<=genus~~~)(.*?)(?=~~~)", entry)[0]
            genus_list.append(genus)

        elif "species" in entry:
            species: str = re.findall("(?<=species~~~)(.*?)(?=~~~)", entry)[0]
            species_list.append(species)
    # print(Counter(genus_list))
    # print(Counter(species_list))
    # final_genus: str = max(Counter(genus_list).items(), key=lambda x: x[1])[0]
    # final_species: str = max(Counter(species_list).items(), key=lambda x: x[1])[0]
...
def write_to_file(output_path: str, info_list: list[str]) -> None:
    """
    ...
    :return: None
    """
    species: str = info_list[0]
    genus: str = info_list[1]
